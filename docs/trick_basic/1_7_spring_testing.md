...
### 1.7 Spring Testing
{: .no_toc }
'테스트'는 소프트웨어 개발의 필수 요소중 하나이며, 'TDD(테스트주도개발)'는 벌써 10년 전부터 소프트웨어 개발자들에게 트렌드 로 떠올랐던것이 이제는 필수가 되었습니다.  이번챕터에서는 우리가 해왔던 테스트를 정말 개발의 일부분으로써 필요한 과정으로써의 테스트로 만들어보고 스프링에서 테스트를 위해 지원해주는 사항들을 알아보도록 하겠습니다.
